function NCO_Phase = Costas_track(k)
    ...
    time_1=evalin('base','time_1');
    RB=evalin('base','RB');
    fc=evalin('base','fc');
    fd=evalin('base','fd');
    N_1=evalin('base','N_1');
    Datacode=evalin('base','Datacode');

    Datacode_1=Datacode((1:N_1)+(k-1)*N_1);

    fs4costas=600e7;
    t4costas=0:1/fs4costas:time_1-1/fs4costas; % 时间序列
    L=length(t4costas);
    %% 构造数字基带信号
    I_Data=sample(Datacode_1,RB,fs4costas,time_1)';
    Q_Data=zeros(L,1,1);
